...
class SportsSpiderSpider(scrapy.Spider):
    name = "sports_spider"
    ...
    def start_requests(self):
        """开始请求 - 获取所有体育项目"""
        # 为所有语言同时发起初始请求，确保体育项目的多语言数据完整
        languages = ['pt', 'es', 'en']
        sync_group = f"sports_{int(time.time())}"
        
        for language in languages:
            base_url = self.base_url_template.format(language)
            sports_url = f"{base_url}/config_sports/41/0"
            yield scrapy.Request(
                url=sports_url,
                headers=self.custom_headers,
                callback=self.parse_sports,
                meta={'step': 'sports', 'language': language, 'sync_group': sync_group}
            )
    ...
    def parse_sports(self, response):
        ...
        try:
            ...
            if 'doc' in data and len(data['doc']) > 0 and 'data' in data['doc'][0]:
                sports_data = data['doc'][0]['data']
            
            self.logger.info(f"获取到 {len(sports_data)} 个体育项目")
            ...
            for sport in sports_data:
                sport_id = sport.get('_id')  # 使用_id字段
                if sport_id in sport_priorities:
                    # 为当前语言创建体育项目Item
                    sport_item = SportItem()
                    sport_item['sport_id'] = sport_id
                    sport_item['sport_name'] = sport.get('name', '')
                    sport_item['sport_sid'] = sport.get('_sid', '')
                    sport_item['language_code'] = language
                    sport_item['created_at'] = datetime.now().isoformat()
                    sport_item['updated_at'] = datetime.now().isoformat()
                    
                    yield sport_item
                    
                    # 只在第一种语言时为所有语言同时生成countries请求
                    if language == 'pt':  # 只在处理第一种语言时生成所有语言的请求
                        sync_group = f"countries_{sport_id}_{int(time.time())}"
                        languages = ['pt', 'es', 'en']
                        for lang in languages:
                            base_url = self.base_url_template.format(lang)
                            countries_url = f"{base_url}/config_tree_mini/41/0/{sport_id}"
                            yield scrapy.Request(
                                url=countries_url,
                                headers=self.custom_headers,
                                callback=self.parse_countries,
                                meta={'sport_id': sport_id, 'language': lang, 'step': 'countries', 'sync_group': sync_group}
                            )
            ...
            self.logger.error(f"解析体育项目数据失败: {e}")
    ...
    def parse_countries(self, response):
        ...
        try:
            ...
            self.logger.info(f"体育项目 {sport_id} 获取到 {len(countries_data)} 个国家/地区")
            ...
            for country in countries_data:
                ...
                if current_language == 'pt':  # 只在处理第一种语言时生成所有语言的请求
                    sync_group = f"leagues_{sport_id}_{country_id}_{int(time.time())}"
                    languages = ['pt', 'es', 'en']
                    for lang in languages:
                        base_url = self.base_url_template.format(lang)
            ...
            self.logger.error(f"解析国家/地区数据失败: {e}")
        ...
        try:
            ...
            for league in leagues_data:
                ...
                if season_id:
                    ...
                    current_language = response.meta.get('language', 'pt')
                    if current_language == 'pt':  # 只在处理第一种语言时生成所有语言的请求
                        sync_group = f"matches_{season_id}_{int(time.time())}"
                        languages = ['pt', 'es', 'en']
                        for lang in languages:
                            base_url = self.base_url_template.format(lang)
            ...
            self.logger.error(f"解析联赛数据失败: {e}")
    ...
    def parse_matches(self, response):
        ...
        try:
            ...
            yield season_item
            self.logger.info(f"赛季 {season_id} 获取到 {len(matches_data)} 场比赛")
            ...
            for match in matches_data:
                ...
                # match_item['home_team_name'] = home_team.get('name', '')
                match_item['away_team_id'] = away_team.get('_id')
                # match_item['away_team_name'] = away_team.get('name', '')
            ...
            self.logger.error(f"解析比赛数据失败: {e}")
    
    def extract_year_from_season(self, season_name):
        """从赛季名称中提取年份"""
        try:
            if '/' in season_name:
                # 格式如 "2023/24"
