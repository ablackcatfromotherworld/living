...
class FootballSpider(scrapy.Spider):
    ...
    def start_requests(self):
        """开始请求 - 直接获取足球的国家数据"""
        languages = ['pt', 'es', 'en']
        sync_group = f"football_countries_{int(time.time())}"
    ...
    def parse_countries(self, response):
        ...
        try:
            ...
            self.logger.info(f"足球运动获取到 {len(countries_data)} 个国家/地区")
            ...
            for country in countries_data:
                ...
                if current_language == 'pt':  # 只在处理第一种语言时生成所有语言的请求
                    sync_group = f"football_leagues_{sport_id}_{country_id}_{int(time.time())}"
                    languages = ['pt', 'es', 'en']
            ...
            self.logger.error(f"解析足球国家/地区数据失败: {e}")
        ...
        try:
            ...
            for league in leagues_data:
                ...
                if season_id:
                    ...
                    current_language = response.meta.get('language', 'pt')
                    if current_language == 'pt':  # 只在处理第一种语言时生成所有语言的请求
                        sync_group = f"football_matches_{season_id}_{int(time.time())}"
                        languages = ['pt', 'es', 'en']
                        for lang in languages:
                            base_url = self.base_url_template.format(lang)
            ...
            self.logger.error(f"解析足球联赛数据失败: {e}")
    ...
    def parse_matches(self, response):
        ...
        try:
            ...
            self.logger.error(f"解析足球比赛数据失败: {e}")
